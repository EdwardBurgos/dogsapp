    ...
    useEffect(() => {
        ...
        function reRender(actualElement, direction) {
            ...
            for (let i = 0; i < 11; i++) {
                ...
                else if (i === 2 && number - 1 > 0) { buttons.push(<button key="pointsInit" disabled id="reRenderLeft">...</button>) }
        ...
        function firstRender(direction) {
            ...
            for (let i = 0; i < 11; i++) {
                ...
                else if (i === 8 && direction === "last") { buttons.push(<button key={pages} id={`link${pages}`} className={s.active} onClick={e => changePageFunction(e)}>{pages}</button>); }
                else if (i === 9) { buttons.push(<button key="next" id="next" onClick={e => navigate(e, "next")}>&gt;</button>); }
                else if (i === 10) { buttons.push(<button key="last" id="last" onClick={e => navigate(e, "last")}>&raquo;</button>); }
                else if (ellipsis && i === 5) { buttons.push(<button key="middle" disabled id="reRender">...</button>) }
                else if (i < 5) { buttons.push(<button key={i - 1} id={`link${i - 1}`} onClick={e => changePageFunction(e)}>{i - 1}</button>); }
                else if (i > 5) { buttons.push(<button key={pages + (i - 8)} id={`link${pages + (i - 8)}`} onClick={e => changePageFunction(e)}>{pages + (i - 8)}</button>); }
            }
            setItems(buttons);
        }
        firstRender();
